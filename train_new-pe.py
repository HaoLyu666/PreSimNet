...
import os
from evaluate_new_pe import Evaluate
from config import *
from DWA import *
import torch as t
import requests
...
def param_distribution_loss(idm_params, cf_probs):
    """消融实验：只处理IDM参数的分布损失"""
    total_loss = 0.0
    ...
    # 处理IDM参数（只有一个专家）
    params = idm_params[0].squeeze(1)  # [B, 6]
    idm_loss = 0
...
def main():
    ...
    # 手动初始化参数预测头的偏置（消融实验：只有一个IDM专家）
    with t.no_grad():
        # 初始化IDM专家的偏置
        expert = Encoder.parameter_head.idm_expert
    ...
    for epoch in range(args['last_epoch'], args['epoch']):
        ...
        for idx, data in enumerate(tqdm(trainDataloader)):
            ...
            # cf_type_pred = model_outputs['cf_type_pred']  # [B, cf_type] # 消融：无类型预测

            # 提取参数
            params_dict = model_outputs['params']
            idm_params = params_dict['idm_params']  # 只有一个IDM专家的参数
            cf_probs = params_dict['cf_probs']  # [B, 1] 消融实验中固定为None
            ...
            # 3. 跟驰类型预测损失（保留CF_type预测功能）
            # cf_type_loss = t.nn.CrossEntropyLoss()(cf_type_pred, cf_type) # 消融：移除

            # 4. 参数分布损失
            param_dist_loss = param_distribution_loss(idm_params, cf_probs)
            ...
            # loss_cf_type += cf_type_loss.item() # 消融：移除
        ...
        avg_losses = {
            ...
            # 'cf_type': loss_cf_type / batch_count, # 消融：移除
