...
def main():
    ...
    for epoch in range(args['last_epoch'], args['epoch']):
        ...
        for idx, data in enumerate(tqdm(trainDataloader)):
            ...
            # 提取预测结果
            direct_pred = predictions['position_pred']  # [B, out_length, 1]
            ...
            # 根据模式调整权重
            if args['ablation_mode'] == 'no_direct':
                # 消融实验1：去掉直接轨迹预测 (Direct Position Prediction)
                w_direct = 0.0
            elif args['ablation_mode'] == 'no_dynamic':
                # 消融实验2：去掉动力学模型/物理引导 (Physics-based Dynamics)
                w_dynamic = 0.0
            elif args['ablation_mode'] == 'no_cls':
                # 消融实验3：去掉跟驰类型分类 (Car-following Classification)
                w_cls = 0.0

            # 组合损失 (应用权重)
            task_losses = {
                'direct_pos': w_direct * direct_pos_loss,



                'cf_type': w_cls * cf_type_loss
            }
