...
class TrajectoryPredictionHead(nn.Module):
    ...
    def forward(self, hist_enc, cf_type_probs, init_pos):
        ...
        # 2. 添加位置编码
        # pos_queries = t.zeros(batch_size, self.out_length, self.encoder_size, device=cv_ca_features.device)
        pos_queries = self.pos_encoder(cv_ca_features)
        cv_ca_features = cv_ca_features + pos_queries
        
        # 3. 直接使用跟驰类型概率向量，在时间维度上复制
        # cf_probs_expanded = cf_type_probs.unsqueeze(1).repeat(1, self.out_length, 1)  # [B, out_length, cf_type]
        
        # 4. 融合跟驰类型概率到查询
        # query = t.cat([cv_ca_features, cf_probs_expanded], dim=-1)
        ...
        decoder_hidden = hist_enc[..., -1, :].unsqueeze(0).repeat(self.num_layers, 1, 1)
        # decoder_hidden = t.zeros(self.num_layers, batch_size, self.encoder_size, device=decoder_input.device)
        ...
        for i in range(self.out_length):
            ...
            decoder_input = cv_ca_features[:, i:i+1, :]
            # 单步解码
            decoder_output, decoder_hidden = self.forward_step(
                decoder_input, decoder_hidden, memory
            )
        ...
        # 9. 堆叠所有输出
        decoder_outputs = t.cat(all_outputs, dim=1)  # [B, out_length, encoder_size]
...
class ParameterPredictionHead(nn.Module):
    def __init__(self, encoder_size, cf_type, dropout, n_haed):
        super().__init__()
        self.encoder_size = encoder_size
        self.cf_type = cf_type
        ...
        # 注意力机制，用于融合历史编码和查询
        self.self_attention = nn.MultiheadAttention(encoder_size, num_heads=4, batch_first=True)
        ...
        # 注意力机制，使用轨迹预测的编码作为KV
        self.cross_attention = nn.MultiheadAttention(encoder_size, num_heads=4, batch_first=True)
        ...
        self.all_experts = nn.ModuleList([
            MLP(f_in=encoder_size, f_out=6, activation='tanh',  # ACC模型参数: kp, kd, v_des, h0, h1, kv
                hidden_dim=encoder_size*2, hidden_layers=2, dropout=self.dropout)
            for _ in range(self.cf_type)
        ])
        
        # 归一化层 - 减少冗余层
        self.layer_norm1 = nn.LayerNorm(encoder_size)  # 输入归一化
        self.layer_norm2 = nn.LayerNorm(encoder_size)  # 自注意力后归一化
        self.layer_norm3 = nn.LayerNorm(encoder_size)  # FFN1后归一化
        self.layer_norm4 = nn.LayerNorm(encoder_size)  # 交叉注意力和FFN2后归一化
    ...
    def forward(self, hist_enc, cf_type_pred, traj_encoding=None):
        ...
        param_query = param_query + self.dropout_layer(attn_output)
        param_query = self.layer_norm2(param_query)
        
        # 前馈网络处理
        ffn_output = self.ffn1(param_query)
        param_query = param_query + self.dropout_layer(ffn_output)
        param_query = self.layer_norm3(param_query)
        
        # 如果有轨迹编码，使用交叉注意力机制增强参数查询
        if traj_encoding is not None:
            ...
            param_query = param_query + self.dropout_layer(cross_attn_output)
            
            # 添加交叉注意力后的FFN
            ffn_output = self.ffn2(param_query)
            param_query = param_query + self.dropout_layer(ffn_output)
            # 只在最后归一化一次
...
class Encoder(nn.Module):
    def __init__(self, args):
        ...
        self.type_pred = MLP(f_in=self.encoder_size,
                               f_out=self.cf_type,
                               activation='tanh',
        ...
        self.parameter_head = ParameterPredictionHead(
            encoder_size=self.encoder_size,
            cf_type=self.cf_type, dropout=self.dropout, n_haed=self.n_head
        )
        
        self.GLU = GLU(self.encoder_size*2, self.encoder_size)
        self.norm = nn.LayerNorm(self.encoder_size)
...
class predictor:
    ...
    def veh_dynamic(self, veh_state, acc_params_list, idm_params_list, cf_probs, next_v):
        ...
        a_t = t.bmm(cf_probs.unsqueeze(1), all_accelerations).squeeze(1)
