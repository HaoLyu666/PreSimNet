...
class ParameterPredictionHead(nn.Module):
    ...
    def __init__(self, encoder_size, cf_type, dropout, n_head):
        ...
        # 可学习的参数查询向量
        self.param_query = nn.Parameter(t.Tensor(1, 20, encoder_size))
        ...
        # 使用原版本的MLP专家网络设计
        self.all_experts = MLP(f_in=encoder_size, f_out=1, activation='relu',
    ...
    def forward(self, hist_enc, cf_type_pred, traj_encoding=None):
        ...
        # 1. 准备参数查询向量
        param_query = self.param_query.repeat(batch_size, 1, 1)  # [B, 20, encoder_size]
        ...
        a_pred = self.all_experts(param_query)
        # # 4. 为每个专家生成参数
        # all_params = []
...
class predictor:
    ...
    def veh_dynamic(self, veh_state, a_pred, next_v):
        ...
        a_t = t.clamp(a_pred, min=-5, max=5)
    ...
    def forward(self, model_outputs, nextv, veh_state):
        ...
        a_pred = params['a_pred'].squeeze()  # 列表，包含ACC模型的参数
